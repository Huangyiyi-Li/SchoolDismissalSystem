...
class LedConfigTests(unittest.TestCase):
    ...
    def test_led_ip_port_and_page_interval_persist(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config.set("led_club_rows_per_group", 5)
            config.set("led_club_groups_per_page", 4)
            ...
            self.assertEqual(reloaded.get("led_club_rows_per_group"), 5)
            self.assertEqual(reloaded.get("led_club_groups_per_page"), 4)
    ...
    def test_defaults_match_current_bx_6e1xp_installation(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(config.get("led_club_rows_per_group"), 4)
            self.assertEqual(config.get("led_club_groups_per_page"), 5)
