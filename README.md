...
* **仰邦 LED 状态屏**:
  * 支持多块屏独立配置，显示不同、相同或部分重叠的年级；可共用显示方案，详见[多屏使用与验收说明](docs/multi-screen/README.md)。
...
* `led_screens` / `led_display_plans`: 屏幕设备列表与可共用的显示方案。多屏配置优先；下列单屏字段用于旧配置迁移和兼容。
* `led_enabled`: 兼容的总启用标记；多屏运行时以各屏的 `enabled` 为准。
...
3. **管理**:
    ...
    * 修改 LED 参数后点击“生成预览”；预览在后台生成，不会因输入参数而反复卡顿。首次接屏先点击“测试连接”，成功后点击“发送测试画面”；测试结束可点击“恢复此屏原节目”；需要重置所有屏幕时使用“重置全校今日放学状态”。
    * 测试正常后为需要使用的屏幕勾选“启用此屏”并保存。行政班放学时段结束后，客户端会删除动态区并恢复 Ledshow 原节目。
